...
exports.SalesTransactionRepository = void 0;
const DataParse_1 = __importDefault(require("../utils/DataParse"));
const BaseRepository_1 = require("./BaseRepository");
const SalesTransactionItemsRepository_1 = require("./SalesTransactionItemsRepository");
const SalesTransactionReceiptRepository_1 = require("./SalesTransactionReceiptRepository");
const SalesTransactionResponseRepository_1 = require("./SalesTransactionResponseRepository");
const date_time_1 = require("../utils/date-time");
class SalesTransactionRepository extends BaseRepository_1.BaseRepository {
    constructor() {
        super();
        this.tableName = 'sales_transactions';
        this.tableReceipt = 'sales_transactions_receipt';
        this.tableTItem = 'sales_transactions_items';
        this.tableResp = 'sales_transactions_response';
        this.salesTransactionItemsRepository = new SalesTransactionItemsRepository_1.SalesTransactionItemsRepository();
        this.salesTransactionReceiptRepository = new SalesTransactionReceiptRepository_1.SalesTransactionReceiptRepository();
        this.salesTransactionresponseRepository = new SalesTransactionResponseRepository_1.SalesTransactionResponseRepository();
    }
    async create(saleTransaction, tx) {
        ...
        if (tx) {
            const result = await tx.queryNamed(sql, saleTransaction);
            return result.insertId;
        }
        else {
            const result = await this.queryNamed(sql, saleTransaction);
            return result.insertId;
        }
    }
    async getLatestInvoiceId(payload) {
    ...
    async getLatestSalesTransactionId(payload) {
    ...
    async checkExistedInvoiceBeforeInsert(tin, bhfId, allInvo, type) {
    ...
    async createWithTransaction(saleTransaction) {
        try {
            let id;
    ...
    async getSalesTransactionByTinAndBhfId(tin, bhfId) {
    ...
    async checkInvoiceExistedWithoutType(tin, bhfId, invcNo) {
    ...
    async getSalesTransactionByTinAndBhfIdAndTypeAndInvcNo(tin, bhfId, type, invcNo, allInvo, freshInvo) {
    ...
    async getAllInv(tin, bhfId, invcNo, type, limit) {
        const sql2 = `
            SELECT 
            tra.*,
        ...
        const data = (0, DataParse_1.default)(result2, (i) => i.detail_customer_sales_transactions_id);
    ...
    async getAllSalesReport(tin, bhfId, startDate, endDate) {
        // get to day date
        let start;
        let end;
        ...
        const startCfmDt = date_time_1.DateUtils.format(start);
        const endCfmDt = date_time_1.DateUtils.format(end);
        const sql2 = `
            SELECT 
            tra.*,
            cust.sales_transactions_id as detail_customer_sales_transactions_id,
    ...
    async checkExistedInvoice(tin, bhfId, type, invcNo) {
...
exports.SalesTransactionRepository = SalesTransactionRepository;
