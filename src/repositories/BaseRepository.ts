...
export interface TransactionInterface {
  ...
  queryNamed: <T = any>(sql: string, params?: { [key: string]: any }) => Promise<T>;
}
export abstract class BaseRepository {
  protected db: DatabaseConnection;
 ...
 protected async query<T = any>(sql: string, params: any[] = []): Promise<T> {
  ...
  protected async queryNamedWithArrays<T = any>(
    sql: string, 
    params: { [key: string]: any } = {}
): Promise<T> {
    ...
    const { results } = await this.db.executeQuery<T>(processedSql, positionalParams);
    return results;
}

   // Create a transaction-aware interface that wraps the connection
  private createTransactionInterface(connection: any): TransactionInterface {
    return {
      query: async <T = any>(sql: string, params: any[] = []): Promise<T> => {
        const [rows] = await connection.execute(sql, params);
        return rows as T;
      },
      queryNamed: async <T = any>(
        sql: string, 
        params: { [key: string]: any } = {}
      ): Promise<T> => {
...
protected async transaction<T>(
   ...
   */
  protected async createOrUpdate<T = any>(
    tableName: string,
    data: { [key: string]: any },
    conflictKeys: string[],
   ...
   */
  protected async createOrUpdateNamed<T = any>(
    tableName: string,
    data: { [key: string]: any },
    conflictKeys: string[],
    updateColumns?: string[]
    ...
    const result = await this.queryNamed<any>(sql, data);
    return result;
  }
   ...
   */
  protected async createMany<T = any>(
    tableName: string,
   ...
   */
  protected async createManyNamed<T = any>(
    tableName: string,
    dataArray: { [key: string]: any }[]
  ): Promise<T> {
    if (!dataArray.length) {
    ...
    const valuePlaceholders = dataArray.map((_, index) => 
      `(${columns.map(col => `:${col}_${index}`).join(', ')})`
    ).join(', ');
    ...
    const result = await this.queryNamed<any>(sql, namedParams);
    return result;
  }
